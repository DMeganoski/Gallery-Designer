...
if (is_object($this->OtherCategories)) {
   ...
   if ($this->OtherCategories->NumRows() == 0) {
      ...
      if ($this->Category->Visible == '1') {
   ?>
      <li>
         <?php
            echo $this->Form->CheckBox('DeleteItems', "Move items in this category to a replacement category.", array('value' => '1'));
         ?>
      </li>
   <?php }
   if ($this->Category->AllowDiscussions == '1') {
      ?>
      <li id="ReplacementWarning"><p class="Warning"><?php echo T('<strong>Heads Up!</strong> Moving items into a replacement category can result in items vanishing (or appearing) if the replacement category has different permissions than the category being deleted.'); ?></p></li>
      <?php
   }
   ?>
